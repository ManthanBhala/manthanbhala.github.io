...
export default function Home() {
  ...
  return (
    <div id="top" className={`min-h-screen bg-gradient-to-br ${bg.gradient} overflow-x-hidden`}>
      ...
      {/* Languages */}
      <section id="publications" className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 relative">
        <div
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="flex items-center gap-3 mb-8">
            <div className={`w-1 h-8 ${accentDivider} rounded-full`}></div>
            <h2 className="text-3xl sm:text-4xl font-bold text-white">{labels.sections.publications}</h2>
          </div>
            ...
            {publications.map((publication, index) => (
              <div key={index} className={`${cardClass} p-6 hover:${border.accent} hover:shadow-lg hover:${accent.glowSoft} transition-all`}>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-lg font-semibold text-white">{publication.name}</h3>
                </div>
                <p className={`text-sm ${text.accent}`}>{publication.level}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm p-8 rounded-xl border border-slate-700/50 shadow-lg">
              ...
              {contactSubmitStatus ? (
                <div
                  className={`p-8 rounded-lg text-center ${contactSubmitStatus.toLowerCase().includes('oops') || contactSubmitStatus.toLowerCase().includes('wrong')
                ...
                >
                  ...
                  <p
                    className={`mb-6 ${contactSubmitStatus.toLowerCase().includes('oops') || contactSubmitStatus.toLowerCase().includes('wrong')
                      ? `${text.accent}`
                      : `${text.accent}`
                      }`}
                  >
      ...
      {resumeRequest.enabled && resumeModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {resumeSubmitStatus ? (
              <div className={`p-6 rounded-lg text-center ${resumeSubmitStatus.toLowerCase().includes('oops') || resumeSubmitStatus.toLowerCase().includes('wrong')
                ? 'bg-red-500/10 border border-red-500/30'
                : `${accent.glowSoft} border ${border.accent}`
                }`} aria-live="polite">
                ...
                <p className={`font-medium mb-4 ${resumeSubmitStatus.toLowerCase().includes('oops') || resumeSubmitStatus.toLowerCase().includes('wrong')
                  ? `${text.accent}`
                  : `${text.accent}`
                  }`}>
                  {resumeSubmitStatus}
